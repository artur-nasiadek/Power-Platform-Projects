...
On the next screen the list of books is displayed. The list can be filtered with the *search book* field by title and author's name, and also with buttons: *read* and *unread*. Additionally, the list can be sorted by title, author's name or book rating. In order to add new book into the books collection the user has to press the *scan to add new book* button, which will open the barcode scanner. After scanning the new book's ISBN barcode, Power Automate flows are activated. In the first flow the ISBN is used to extract the book's details from Google Books, and then the extracted information is populated in the Dataverse table. Unfortunately, using the URL for the book's cover image did not work on a mobile phone and the cover was not displayed (it worked fine in the development Power Apps Studio, but not on a phone and I cound not find the reason for that). To tackle this issue the second Power Automate flow is used to extract and upload the cover image into the Dataverse table. The new book is displayed on the list of books. 
<br>
<br>
...
<br>
<p><b>Power Automate flow to extract book's details</b></p>
<img src="/Personal Library Manager/Images/FetchBookInfo.png" width="200">
<br>
<p><b>Power Automate flow to upload book's cover image</b></p>
<img src="/Personal Library Manager/Images/UploadBookCoverImage.png" width="200">
